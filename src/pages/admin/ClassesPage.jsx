...
export default function ClassesPage() {
  ...
  const [catalogMatieres, setCatalogMatieres] = useState([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [deleteConfirmId, setDeleteConfirmId] = useState(null);
  const [editingId, setEditingId] = useState(null);
  const [form, setForm] = useState({ nom: '', niveau: '', niveauEtudeId: '', cycle: 'Collège', capacite: 30, montant_annuel: 0, anneeScolaireId: '', tranches: [], matieresIds: [] });
  ...
  const loadData = () => {
    ...
    api.get('/admin/matieres/catalog').then(d => { if (Array.isArray(d)) setCatalogMatieres(d); }).catch(() => {});
  };

  useEffect(() => { loadData(); }, []);

  const toggleMatiereId = (id) => {
    setForm((prev) => {
      const sid = String(id);
      const has = prev.matieresIds.map(String).includes(sid);
      return {
        ...prev,
        matieresIds: has
          ? prev.matieresIds.filter((mId) => String(mId) !== sid)
          : [...prev.matieresIds, id],
      };
    });
  };
  ...
  const validateMatieresForm = () => {
    if (!form.matieresIds?.length) {
      return 'Sélectionnez au moins une matière pour cette classe.';
    }
    return null;
  };
  ...
  const openCreateModal = () => {
    ...
    setForm({
      ...
      anneeScolaireId: activeAnnee ? activeAnnee.id : '', tranches: [], matieresIds: [],
    });
    setEditingId(null);
    setMessage('');
    setIsModalOpen(true);
  ...
  const handleEdit = (c) => {
    setForm({
      ...
      tranches: c.tranches || [],
      matieresIds: (c.matieres || [])
        .map((m) => {
          if (m.catalogMatiereId || m.catalogMatiere?.id) {
            return m.catalogMatiereId || m.catalogMatiere?.id;
          }
          const cat = catalogMatieres.find((cat) => cat.nom === m.nom);
          return cat?.id;
        })
        .filter(Boolean),
    });
                ...
                <div style={{ marginBottom: '1rem' }}>
                  <div style={{ marginBottom: '0.75rem' }}>
                    ...
                    <p style={{ fontSize: '0.8rem', color: '#64748b', margin: '0.35rem 0 0' }}>
                      Les élèves de cette classe seront évalués uniquement sur les matières cochées. Créez d&apos;abord les matières dans le catalogue si besoin.
                    </p>
                  </div>

                  {catalogMatieres.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '1.5rem', background: '#fffbeb', borderRadius: '8px', color: '#92400e', fontSize: '0.9rem', border: '1px solid #fde68a' }}>
                      Aucune matière dans le catalogue. Ajoutez-en depuis la page <strong>Matières</strong>.
                    </div>
                  ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.5rem' }}>
                      {catalogMatieres.map((m) => {
                        const checked = form.matieresIds.map(String).includes(String(m.id));
                        return (
                          <label
                            key={m.id}
                            style={{
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={() => toggleMatiereId(m.id)}
                              style={{ width: '16px', height: '16px', accentColor: '#0A2F6B' }}
                            />
                            <span>
                              <strong>{m.nom}</strong>
                              <span style={{ color: '#64748b', marginLeft: '0.35rem' }}>×{m.coefficient}</span>
                            </span>
                          </label>
                        );
                      })}
                  ...
                  {form.matieresIds.length > 0 && (
                    <p style={{ marginTop: '0.75rem', fontSize: '0.82rem', color: '#0A2F6B', fontWeight: 600 }}>
                      {form.matieresIds.length} matière{form.matieresIds.length > 1 ? 's' : ''} sélectionnée{form.matieresIds.length > 1 ? 's' : ''}
                    </p>
                  )}
                </div>
              </form>
